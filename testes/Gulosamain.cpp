...
int main() {
  ...
  double tempo_total, r;

  //  >>>>>>>>>>>>>>>>>>>>>   BATCH 8
  get_names(batch, "instancias_c/batch8");

  arquivo.open("saida/Gulosa_batch8.txt");
  for (std::string nome : batch) {
    ...
    arquivo << nome << "-" << tempo_total << "-" << r << std::endl;
  }
  ...
  arquivo.open("saida/Gulosa_batch9.txt");
  for (std::string nome : batch) {
  ...
  arquivo.open("saida/Gulosa_batch10.txt");
  for (std::string nome : batch) {
  ...
  arquivo.open("saida/Gulosa_batch11.txt");
  for (std::string nome : batch) {
